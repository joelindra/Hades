...
try:
    ...
    roots = [Path("."), Path(__file__).parent.parent.parent]
    for root in roots:
        env_file = root / ".env"
        if env_file.exists():
    ...
    pass  # Silently fail if .env loading fails

from hades.interface.cli import run_cli
from hades.interface.tui import run_tui
from hades.interface.utils import (
...
from hades.runtime.docker_runtime import HADES_IMAGE
...
def validate_environment() -> None:  # noqa: PLR0912, PLR0915
    ...
    model = os.getenv("HADES_LLM", "")
    if not model:
        missing_required_vars.append("HADES_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "HADES_LLM":
                ...
                error_text.append("HADES_LLM", style="bold cyan")
                error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export HADES_LLM='openai/gpt-5'\n", style="dim white")
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  HADES CONFIGURATION ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
def check_docker_installed() -> None:
    if shutil.which("docker") is None:
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  HADES STARTUP ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm() -> None:
    ...
    try:
        model_name = os.getenv("HADES_LLM", "openai/gpt-5")

        # Map generic LLM_API_KEY to provider-specific key if needed
        generic_key = os.getenv("LLM_API_KEY")
        if generic_key:
            if model_name.startswith("gemini/") and not os.getenv("GOOGLE_API_KEY"):
                os.environ["GOOGLE_API_KEY"] = generic_key
                ...
                os.environ["OPENAI_API_KEY"] = generic_key
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hades Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  hades --target https://example.com

  # GitHub repository analysis
  hades --target https://github.com/user/repo
  hades --target git@github.com:user/repo.git

  # Local code analysis
  hades --target ./my-project

  # Domain penetration test
  hades --target example.com

  # Multiple targets (e.g., white-box testing with source and deployed app)
  hades --target https://github.com/user/repo --target https://example.com
  hades --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Custom instructions
  hades --target example.com --instruction "Focus on authentication vulnerabilities"
        """,
    )
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    panel = Panel(
        panel_content,
        title="[bold green]🛡️  HADES CYBERSECURITY AGENT",
        title_align="center",
...
def pull_docker_image() -> None:
    ...
    if image_exists(client, HADES_IMAGE):
        return

    console.print()
    console.print(f"[bold cyan]🐳 Pulling Docker image:[/] {HADES_IMAGE}")
    ...
    with console.status("[bold cyan]Downloading image layers...", spinner="dots") as status:
        try:
            ...
            for line in client.api.pull(HADES_IMAGE, stream=True, decode=True):
            ...
            error_text.append("\n\n", style="white")
            error_text.append(f"Could not download: {HADES_IMAGE}\n", style="white")
