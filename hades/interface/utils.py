...
def infer_target_type(target: str) -> tuple[str, dict[str, str]]:
    ...
    # Check if target looks like a domain with typo (comma instead of dot)
    if "," in target:
        raise ValueError(
            f"Invalid target: {target}\n"
            "Did you mean to use a dot (.) instead of a comma (,)?\n"
            f"Example: If you meant '{target.replace(',', '.')}', use that instead."
        )
    ...
    raise ValueError(
        ...
        "- A domain name (e.g., example.com, protergo.id)"
    )
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "hades_repos" / run_name
    ...
    try:
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  HADES CLONE ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
