...
from hades.agents.HadesAgent import HadesAgent
from hades.llm.config import LLMConfig
from hades.telemetry.tracer import Tracer, set_global_tracer
...
def get_package_version() -> str:
    try:
        return pkg_version("hades-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: HadesTUIApp | None = None

    def set_app_reference(self, app: "HadesTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    CYBER_GREEN   = "#00ff9f"  # Vaporware Green
    MATRIX_GREEN  = "#00ff41"
    DEEP_GREEN    = "#003b00"
    NEON_BLUE     = "#00d4ff"
    CRITICAL_RED  = "#ff003c"
    ...
    BOOT_SEQUENCE = [
        ("CORE.SYS", "INITIALIZING NEURAL LINK...", "ok"),
        ("NET.IFC", "ESTABLISHING STEALTH TUNNEL...", "warn"),
        ("MEM.DB",  "MOUNTING EXPLOIT VAULT...", "ok"),
        ("AI.CORE", "WAKING HADES INFERENCE...", "ok"),
        ("SEC.BYP", "INJECTING ANTI-WAF MODULE...", "hi"),
        ("GHOST.V", "STEALTH PROTOCOL ACTIVATED", "warn"),
        ("SYSTEM",  "HADES vPro READY FOR OPS", "hi"),
    ]
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._boot_timer:      Timer | None = None
        self._panel_static:   Static | None = None
        self._glitch_active  = False

    def compose(self) -> ComposeResult:
        panel_static = Static(self._build_panel(), id="splash_content")
        self._panel_static = panel_static
        yield panel_static

    def on_mount(self) -> None:
        self._animation_timer = self.set_interval(0.3, self._animate)
        self._boot_timer = self.set_interval(0.4, self._advance_boot)

    def on_unmount(self) -> None:
        for timer in (self._animation_timer, self._boot_timer):
            if timer is not None:
                timer.stop()

    def _animate(self) -> None:
        if not self._panel_static: return
        self._animation_step += 1
        self._metrics_tick = random.randint(0, 100)  # nosec B311
        self._glitch_active = random.random() < 0.2
        self._panel_static.update(self._build_panel())

    def _advance_boot(self) -> None:
        if self._boot_step < len(self.BOOT_SEQUENCE):
            self._boot_step += 1
            if self._panel_static:
                self._panel_static.update(self._build_panel())

    def _build_panel(self) -> Panel:
        # Create a layout with Header, Main Content (Banner + Info), and Boot Status
        header = self._build_header()
        banner = self._build_banner()
        metrics = self._build_metrics_grid()
        boot_log = self._build_boot_log()
        ...
        return Panel(
            content,
            border_style=Style(color="#334155"), # Slate border for modern look
            padding=(1, 4),
        )

    def _build_header(self) -> Text:
        t = Text()
        status_style = Style(color=self.CYBER_GREEN, bold=True)
        dim_style    = Style(color=self.TEXT_DIM)
        
        t.append(" ● ", style=status_style)
        t.append("HADES_TERMINAL", style=status_style)
        t.append(" | ", style=dim_style)
        t.append("DOCKER_RUNTIME: ", style=dim_style)
        t.append("ACTIVE", style=Style(color=self.NEON_BLUE, bold=True))
        t.append(" | ", style=dim_style)
        t.append("SECURITY_CLEARANCE: ", style=dim_style)
        t.append("ROOT", style=Style(color=self.CRITICAL_RED, bold=True))
        return t
    ...
    def _build_banner(self) -> Text:
        ...
        if self._glitch_active:
            ...
            for _ in range(3):
                ...
                if text[idx] not in (" ", "\n"):
                    text = text[:idx] + random.choice(self._GLITCH_CHARS) + text[idx+1:]
        
        return Text(text.strip("\n"), style=Style(color=self.CYBER_GREEN, bold=True), justify="center")

    def _build_metrics_grid(self) -> Text:
        t = Text()
        label = Style(color=self.TEXT_DIM)
        val   = Style(color=self.NEON_BLUE, bold=True)
        
        items = [
            ("🧠 NEURAL_LINK", f"{80 + (self._metrics_tick % 20)}%"),
            ("📶 LATENCY",     f"{10 + (self._metrics_tick % 15)}ms"),
            ("🔥 CPU_LOAD",    f"{15 + (self._metrics_tick % 5)}%"),
            ("📡 GHOST_NET",   "READY"),
        ]
        
        for i, (l, v) in enumerate(items):
            t.append(f" {l:<15}", style=label)
            t.append(f"{v:<15}", style=val)
            if i % 2 == 1 and i < len(items) - 1:
                t.append("\n")
        return t

    def _build_boot_log(self) -> Text:
        t = Text()
        visible = self.BOOT_SEQUENCE[:self._boot_step]
        for tag, msg, kind in visible:
            tag_style = Style(color=self.TEXT_DIM, bold=True)
            msg_style = Style(color=self.CYBER_GREEN) if kind == "ok" else \
                        Style(color=self.WARNING_GOLD) if kind == "warn" else \
                        Style(color=self.NEON_BLUE, bold=True)
            
            t.append(f"[{tag:<8}] ", style=tag_style)
            t.append(f"{msg}\n", style=msg_style)
        
        # Padding
        for _ in range(len(self.BOOT_SEQUENCE) - len(visible)):
            t.append("·" * 40 + "\n", style=Style(color=self.DEEP_GREEN))
        return t

    def _build_footer(self) -> Text:
        pulse = self._animation_step % 2 == 1
        t = Text("▶ STARTING_AGENTS", style=Style(color="white", bold=pulse, dim=not pulse))
        t.append("...", style=Style(color=self.CYBER_GREEN, blink=True))
        t.append(" █", style=Style(color=self.CYBER_GREEN, blink=True))
        return t

    def _build_welcome_text(self) -> Text:
        t = Text("WELCOME TO ", style=Style(color="white", bold=True))
        t.append("HADES", style=Style(color=self.PRIMARY_GREEN, bold=True))
        t.append("!", style=Style(color="white", bold=True))
        return t

    def _build_tagline_text(self) -> Text:
        return Text(
            "// OPEN-SOURCE AI CYBERSECURITY AGENT //",
            style=Style(color=self.DIM_GREEN, dim=True),
        )

    def _build_start_line(self) -> Text:
        """Pulsing 'Starting Hades…' line with blinking cursor."""
        emphasize  = self._animation_step % 2 == 1
        base_style = Style(color="white", dim=not emphasize, bold=emphasize)
        hades_style = Style(color=self.PRIMARY_GREEN, bold=bool(emphasize))

        t = Text("▶ Starting ", style=base_style)
        t.append("Hades", style=hades_style)
        t.append(" Cybersecurity Agent", style=base_style)
        # Blinking block cursor
        cursor_style = Style(
            color=self.PRIMARY_GREEN,
            bold=True,
            blink=emphasize,
        )
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("🦉 Hades Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("🦉 Quit Hades? ", id="quit_title"),
            Grid(
...
class HadesTUIApp(App):  # type: ignore[misc]
    ...
    def _start_scan_thread(self) -> None:
        def scan_target() -> None:
            try:
                ...
                try:
                    agent = HadesAgent(self.agent_config)
    ...
    def _render_chat_content(self, msg_data: dict[str, Any]) -> str:
        ...
        if role == "user":
            from hades.interface.tool_components.user_message_renderer import UserMessageRenderer
    ...
    def _render_tool_content_simple(self, tool_data: dict[str, Any]) -> str:
        ...
        from hades.interface.tool_components.registry import get_tool_renderer

        renderer = get_tool_renderer(tool_name)

        if renderer:
            widget = renderer.render(tool_data)
            # Get content from Static widget
            # Static widget in Textual stores content as string
            try:
                # Try accessing renderable (Rich renderable object) - for Rich markup
                if hasattr(widget, 'renderable') and widget.renderable is not None:
                    # If renderable is a string, use it directly
                    if isinstance(widget.renderable, str):
                        content = widget.renderable
                    else:
                        # If it's a Rich object, render it
                        from rich.console import Console
                        from io import StringIO
                        console = Console(file=StringIO(), width=120, force_terminal=False)
                        console.print(widget.renderable)
                        content = console.file.getvalue().strip()
                # Try accessing _text (internal text storage in some Textual versions)
                elif hasattr(widget, '_text') and widget._text is not None:
                    content = str(widget._text)
                # Try accessing content property
                elif hasattr(widget, 'content') and widget.content is not None:
                    content = str(widget.content)
                # Try accessing render_str method
                elif hasattr(widget, 'render_str'):
                    content = widget.render_str()
                # Try accessing the first argument passed to Static constructor
                elif hasattr(widget, '_static_content'):
                    content = str(widget._static_content)
                else:
                    # Last resort: try to get from widget's internal state
                    # Static widget typically stores content in _text or similar
                    content = f"→ Using tool [bold blue]{tool_name}[/]"
            except Exception:
    ...
    def _send_user_message(self, message: str) -> None:
        ...
        try:
            from hades.tools.agents_graph.agents_graph_actions import send_user_message_to_agent
    ...
    def action_confirm_stop_agent(self, agent_id: str) -> None:
        ...
        try:
            from hades.tools.agents_graph.agents_graph_actions import stop_agent
...
async def run_tui(args: argparse.Namespace) -> None:
    """Run hades in interactive TUI mode with textual."""
    app = HadesTUIApp(args)
    await app.run_async()
