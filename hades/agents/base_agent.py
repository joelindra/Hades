...
if TYPE_CHECKING:
    from hades.telemetry.tracer import Tracer
...
from hades.llm import LLM, LLMConfig, LLMRequestFailedError
from hades.llm.utils import clean_content
from hades.tools import process_tool_invocations
...
class BaseAgent(metaclass=AgentMeta):
    ...
    def _add_to_agents_graph(self) -> None:
        from hades.tools.agents_graph import agents_graph_actions
    ...
    async def _wait_for_input(self) -> None:
        ...
        if self.state.has_waiting_timeout():
            ...
            try:
                from hades.tools.agents_graph.agents_graph_actions import _agent_graph
    ...
    async def _process_iteration(self, tracer: Optional["Tracer"]) -> bool:
        ...
        self.state.add_message("assistant", response.content, reasoning_details=response.reasoning_details)
        if tracer:
    ...
    def _check_agent_messages(self, state: AgentState) -> None:  # noqa: PLR0912
        try:
            from hades.tools.agents_graph.agents_graph_actions import _agent_graph, _agent_messages
