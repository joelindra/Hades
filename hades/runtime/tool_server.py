...
if not SANDBOX_MODE:
    raise RuntimeError("Tool server should only run in sandbox mode (HADES_SANDBOX_MODE=true)")

parser = argparse.ArgumentParser(description="Start Hades tool server")
...
def agent_worker(_agent_id: str, request_queue: Queue[Any], response_queue: Queue[Any]) -> None:
    ...
    from hades.tools.argument_parser import ArgumentConversionError, convert_arguments
    from hades.tools.registry import get_tool_by_name
