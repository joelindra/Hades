...
class MemoryCompressor:
    def __init__(
        ...
        self.model_name = model_name or os.getenv("HADES_LLM", "openai/gpt-5")

        if not self.model_name:
            raise ValueError("HADES_LLM environment variable must be set and not empty")
    ...
    def compress_history(
        ...
        Args:
            ...
            max_messages: Optional maximum number of messages to keep (for rate-limited models)
        """
        if not messages:
            return messages
        ...
        # If max_messages is specified, limit the number of regular messages
        if max_messages is not None:
            if len(regular_msgs) > max_messages:
                regular_msgs = regular_msgs[-max_messages:]
            
            # Aggressively truncate individual message content for rate-limited models
            # to prevent "request too large" errors on Groq/Gemini
            for msg in regular_msgs:
                content = msg.get("content")
                if isinstance(content, str) and len(content) > 750:
                    msg["content"] = content[:750] + "... [TRUNCATED DUE TO RATE LIMIT]"
                elif isinstance(content, list):
                    for item in content:
                        if isinstance(item, dict) and item.get("type") == "text":
                            text = item.get("text", "")
                            if len(text) > 750:
                                item["text"] = text[:750] + "... [TRUNCATED DUE TO RATE LIMIT]"

        # Adjust MIN_RECENT_MESSAGES if max_messages is set and smaller
        min_recent = MIN_RECENT_MESSAGES
        if max_messages is not None and max_messages < MIN_RECENT_MESSAGES:
            min_recent = max(1, max_messages // 2)  # Keep at least half of max_messages as recent

        recent_msgs = regular_msgs[-min_recent:] if len(regular_msgs) > min_recent else regular_msgs
        old_msgs = regular_msgs[:-min_recent] if len(regular_msgs) > min_recent else []
